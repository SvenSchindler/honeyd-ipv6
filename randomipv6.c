...
#define MAX_RANDOM_PER_ATTACKER 1

/**
* store for all attackers which received a randomly generated template
*/
struct attacker_addr {
        SPLAY_ENTRY(attacker_addr) node;
        TAILQ_ENTRY(attacker_addr) next;
        struct addr addr;
        int random_created_count;
};

int attacker_addr_cmp(struct attacker_addr *a, struct attacker_addr *b) {
        return addr_cmp(&a->addr, &b->addr);
}

SPLAY_HEAD(random_request_attacker_tree, attacker_addr) random_request_attacker_tree_head;
SPLAY_PROTOTYPE(random_request_attacker_tree, attacker_addr ,node, attacker_addr_cmp);
SPLAY_GENERATE(random_request_attacker_tree, attacker_addr, node, attacker_addr_cmp);
 ...
 */
int random_create_ipv6_template(const char *template_name, struct addr * attacker_source, const struct interface *inter,float randomipv6_percentage,unsigned long long max_random_ipv6_hosts, FILE *logfp)
{
...
/*check if source already created maximum of allowed hosts*/
    struct attacker_addr tmp;
    tmp.addr = *attacker_source;
    struct attacker_addr * existing_attacker_addr = SPLAY_FIND(random_request_attacker_tree,&random_request_attacker_tree_head,&tmp);
    if (existing_attacker_addr == NULL )
    {
        struct attacker_addr * attacker = calloc(1,sizeof(struct attacker_addr));
        attacker->addr=*attacker_source;//increment later when we are sure we created a template
        SPLAY_INSERT(random_request_attacker_tree, &random_request_attacker_tree_head, attacker);
    } else {
        if (existing_attacker_addr->random_created_count >= MAX_RANDOM_PER_ATTACKER) {
                syslog(LOG_DEBUG, "max number of dynamic templates for attacker reached");
                return 0;
        }
    }
	...
	existing_attacker_addr = SPLAY_FIND(random_request_attacker_tree,&random_request_attacker_tree_head,&tmp);
        existing_attacker_addr->random_created_count++;
        return 1;
    }
    else
    {
