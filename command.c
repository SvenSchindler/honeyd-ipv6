...
{
	if (cmd->pfd == -1 || !cmd->fdconnected) {
		syslog(LOG_DEBUG,"%s: no need to trigger write, descriptor not set or not connected for length %d",__func__,size);
		return;
	}
	if (size) {
		syslog(LOG_DEBUG,"%s: triggering write for size %d", __func__,size);
...
{
	syslog(LOG_DEBUG, "cmd: %x, pread: %x, pwirte: %x", cmd, &cmd->pread, &cmd->pwrite);
...
int cmd_setpriv(struct template *tmpl)
{
	...
	/* XXX: causes problems in valgrind */
