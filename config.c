...
template_find(const char *name)
{
	...
	if(strncmp(name,solicited_node_prefix,strlen(solicited_node_prefix))==0){
		...
		/* XXX this is working but a bit sloppy, if two hosts share the same multicast
	...
	struct template *r = SPLAY_FIND(templtree, &templates, &tmp);
	return r;
}
...
template_deallocate(struct template *tmpl)
{
	...
	if (tmpl->person != NULL)
        ...
        if (tmpl->person6 != NULL)
                personality6_declone(tmpl->person6);
	...
	syslog(LOG_DEBUG, "deallocating template with name %s", tmpl->name);
	free(tmpl->name);
	free(tmpl);
}
    ...
    const struct interface *inter, int start)
{
	...
	if (tmpl->person)
        ...
        if (tmpl->person6)
                newtmpl->person6 = personality6_clone(tmpl->person6);
