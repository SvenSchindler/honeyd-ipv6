...
struct template
{
	...
	struct personality6 *person6;

	int id;
	uint32_t seq;
	int seqcalls;
	struct timeval tv_seq;
...
#define template_free(x)	do {					\
	...
	(x)->refcnt--; 						        \
	/*syslog(LOG_DEBUG,"ref counter decreased by %s to %u",__func__,(x)->refcnt);*/ \
	if ((x)->refcnt <= 0) {					\
	    syslog(LOG_DEBUG,"deallocating template!"); \
        /*template_deallocate(x); XXX we cannot do this currently because we are not maintaining a template counter the same way as in the ipv4 flow*/\
	}				\
} while (0)

static __inline struct template *
template_ref(struct template *tmpl, char * caller)
{
	if (tmpl != NULL)
	{
		tmpl->refcnt++;
		//syslog(LOG_DEBUG,"ref cnt increased to %u by %s",tmpl->refcnt,caller);
		assert(tmpl->refcnt);
	}
	return (tmpl);
}
