...
%token	PERSONALITY PERSONALITY6 RANDOM ANNOTATE NO FINSCAN FRAGMENT DROP OLD NEW COLON
%token	PROXY TRANSPARENT UPTIME DROPRATE IN SYN UID GID ROUTE ENTRY LINK NET UNREACH
...
%token  RANDOMIPVS RANDOMEXCLUDE MANAGED HIH
%token  DBFILE 
%token  SHELLCODEDIR
%token  SUBMISSION
%token	<string> STRING
...
set		: SET template DEFAULT PROTO ACTION action
	...
	} 	| SET template PERSONALITY6 personality6
        {
                if ($2 == NULL || $4 == NULL)
                        break;
                $2->person6 = personality6_clone($4);
...
submission: SUBMISSION STRING
    {
        submission_set_service($2);
    }
;

shellcodedir: SHELLCODEDIR SLASH STRING
    {
        database_set_shellcodedir(add_slash($3));
    }
;

dbsettings: DBFILE SLASH STRING
    {
        database_set_dbfile(add_slash($3));
    }
;
...
hihdefinition	: HIH STRING STRING
	{
		define_hih($2,$3);
	}
;
...
action		: flags STRING
		...
		| flags PROXY ipaddr COLON STRING
	{
		...
		| flags PROXY MANAGED STRING
	{
		char proxy[1024];
		memset(&$$, 0, sizeof($$));
		$$.status = PORT_PROXY_MANAGED;
		$$.action = NULL;
		$$.aitop = NULL;
		$$.flags = $1;
		$$.hih_id = $4;
			...
			| flags PROXY TRANSPARENT
	{
		char proxy[1024];
		memset(&$$, 0, sizeof($$));
		$$.status = PORT_PROXY_MANAGED_TRANSPARENT;
		$$.action = NULL;
		$$.aitop = NULL;
		$$.flags = $1;
	...
	| flags PROXY TRANSPARENT STRING
	{
		char proxy[1024];
		memset(&$$, 0, sizeof($$));
		$$.status = PORT_PROXY_MANAGED_TRANSPARENT;
...
;
personality6    : CMDSTRING
        {
                $1[strlen($1) - 1] = '\0';
                $$ = personality6_find($1+1);
                if ($$ == NULL)
                        yyerror("Unknown personality6 \"%s\"", $1+1);
                free($1);
        }
                | RANDOM
        {
