...
if($srcdirectory =~ m/\/\//){
	...
	$numbers = length(split(/\//,$srcdirectory));
	$zeros = "";
	for($i=0;$i<6-$numbers;$i++){
		$zeros.="/0";
	}
	$zeros.="/";#zero cant be last number	
	$srcdirectory =~ s/\/\//$zeros/;
}

# Randomize Results
$none = 0;
$dstipaddressforrand = $dstipaddress;
$dstipaddressforrand =~ s/\:\:/\:/g;
$dstipaddressforrand =~ s/\:/\./g;
print STDERR $dstipaddressforrand;
@nrs = split(/\./, $dstipaddressforrand);
for $nr (@nrs) {
    $none *= 257;
    $none += hex($nr);
}
$ntwo = 0;
$srcipaddressforrand = $srcipaddress;
$srcipaddressforrand =~ s/\:\:/\:/g;
$srcipaddressforrand =~ s/\:/\./g;
@nrs = split(/\./, $srcipaddressforrand);
...
unless ( 1 or eval "use Mail::Sendmail 0.75; 1" )
{
