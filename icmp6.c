...
struct template *(*find_template)(const char *) = template_find;
...
SPLAY_HEAD(ndp_req_tree,ndp_neighbor_req)
ndp_reqs;
...
/* multicast groups */
SPLAY_HEAD(multicast_group_tree,multicast_group)
multicast_groups;
...
SPLAY_PROTOTYPE(multicast_group_tree, multicast_group, next,
                multicast_group_compare);
SPLAY_GENERATE(multicast_group_tree, multicast_group, next,
               multicast_group_compare);
...
/* TODO: currently we are supporting just one default router */
...
find_or_create_neighbor_entry_struct(struct addr * target_ip_addr)
{
                 ...
                 struct addr *target_ip_addr)
{
    ...
    ret = find_or_create_neighbor_entry_struct(target_ip_addr);
    ret->inter = inter;
...
void send_router_solicitation_cb(int fd, short event, void *arg)
{
    ...
    {
	//do not send a router solicitation on our bridge interface
	if (strcmp(interface_get(i)->if_ent.intf_name, "br0") != 0) {
...
void ndp_init(int send_router_solicitation)
{
    SPLAY_INIT(&ndp_reqs);
    if (send_router_solicitation){
                   ...
                   struct icmp6_hdr * icmp6)
{
    ...
    {
        syslog(LOG_DEBUG, "INFO: icmp packet with invalid checksum received");
        return;
    }
    ...
    case ICMP6_NODE_INFORMATION_QUERY:
        handle_node_information_query(inter, ip6, icmp6);
	break;
    case ICMP6_ECHO_REQUEST:
        handle_echo_request(inter, ip6, icmp6);
        break;
    default:
        syslog(LOG_DEBUG, "INFO: unhandled icmp6 type: %d", icmp6->icmp6_type);
        break;
    }
...
/**
 * Searches through neighbor solicitation options for an ehternet address and returns the first one found.
 */
                                  ...
                                  struct ip6_hdr *ip6, struct icmp6_hdr *icmp6)
{
    ...
    {
        syslog(LOG_DEBUG, "INFO: received a neighbor solicitation for %s from %s",
    ...
    {
        syslog(LOG_DEBUG,
               "INFO: Cant handle neighbor solicitation because no ethernet address for template %s configured.",
               template_name);
    }
    else
    {
        ...
        if (dst_eth_addr != NULL) {
        	...
        	syslog(LOG_DEBUG, "INFO: neighbor solicitation without link layer option, wont send neighbor advertisement");
        }
...
void bytes_to_ethernet_addr(struct addr * ethernet_addr, char * bytes)
{
    ...
    /* TODO: check if there are more options possible */
                                   ...
                                   struct ip6_hdr *ip6, struct icmp6_hdr *icmp6)
{
    ...
    syslog(LOG_DEBUG, "INFO: received neighbor advertisement for %s",
           addr_ntoa(&target_ip_addr));
    /* TODO, very important!!! more headers might be possible, check if packet contains option with target link layer */
    ...
    /* TODO: do we want to process advertisements triggered by others?
     * we may want to remove that check depending on our test cases */

    req = ndp_neighbor_new(inter, &source_mac_addr, &source_ip_addr,
                           &target_mac_addr, &target_ip_addr);
    syslog(LOG_DEBUG,"INFO: added mac address %s for %s to neighbor tree",addr_ntoa(&target_mac_addr),addr_ntoa(&target_ip_addr));
...
void store_in_router_cache(struct router_advertisement *advertisement)
{
    ...
    syslog(LOG_DEBUG, "INFO: received router advertisement for prefix %s",
           addr_ntoa(&router_adv->prefix));
}

/*
 * Processes a router advertisement and stores the received address
 * in a router cache (which is currently just one address)
 */
                                 ...
                                 struct ip6_hdr *ip6, struct icmp6_hdr *icmp6)
{
    ...
    while (processed_bytes < plen)
    {
        ...
        case ND_OPT_PREFIX_INFORMATION:
            ...
            new_router_adv->prefix_len =
            ((struct nd_opt_prefix_info *) opt)->nd_opt_pi_prefix_len;
                                ...
                                struct addr *dst_eth_addr)
{
    ...
    memcpy(&dst_eth_addr->addr_eth, ((uint8_t *) (&ip6_addr->addr_data8)) + 10,
           ETH_ADDR_LEN);
                             ...
                             void (*cb)(struct ndp_neighbor_req *, int, void *), void *arg)
{
    ...
    syslog(LOG_DEBUG, "INFO: sending neighbor solicitation for %s",
...
void icmp6_send_router_sol(const struct interface *inter)
{
    ...
    syslog(LOG_DEBUG, "INFO: sending router solicititation on %s",
    ...
    icmp6_send_pkt(inter, &src_eth_addr, &dst_eth_addr,
                   (struct in6_addr*) (&src_ip_addr.addr_data8),
                   (struct in6_addr*) (&dst_ip_addr.addr_data8), pkt, pkt_len, 0);
}
                             ...
                             struct in6_addr *dst_ip6)
{
    ...
    int add_option = 1;  /* as default add option and target link layer address */

    in6_addr_to_addr(&src_ip_addr, src_ip6);
    struct template *tmpl = template_find(addr_ntoa(&src_ip_addr));
    struct ip_personate6 *pers = NULL;
    if (tmpl != NULL && tmpl->person6 != NULL) {
        pers = &tmpl->person6->ns;
        tc = pers->tc;
        syslog(LOG_DEBUG, "found NS nmap fingerprinting probe PLEN: %u TC: %u", pers->plen, pers->tc);
        if (!personality6_ip6_response(pers))
            return;
    }

    if (pers != NULL && pers->plen != (uint16_t) -1 && pers->plen < 29)
        add_option = 0;

    int pkt_len = sizeof(struct nd_neighbor_advert);

    if (add_option)
        pkt_len += sizeof(struct nd_opt_hdr) + ETH_ADDR_LEN;

    if (pers != NULL)
        syslog(LOG_DEBUG, "plen: %u pkt_len: %u", pers->plen, pkt_len);

    /* our icmp6 package will be 24 or 32 byte long */
    u_char pkg[pkt_len];
    ...
    icmp6_send_pkt(inter, src_eth, dst_eth, src_ip6, dst_ip6, pkg, pkt_len, tc);
                         ...
                         struct icmp6_hdr *icmp6)
{
    ...
    int data_field_len, original_payload_len;
    uint8_t tc = 0;
    ...
    /* test for nmap fingerprint */
    struct ip_personate6 *pers = NULL;
    if (tmpl != NULL && tmpl->person6 != NULL && icmp6->icmp6_id == htons(0xabcd)) {
        if (icmp6->icmp6_code == 9 && icmp6->icmp6_seq == htons(0)) {
            pers = &tmpl->person6->ie[0];
            tc = pers->tc;
            syslog(LOG_DEBUG, "found IE1 nmap fingerprinting probe P:%u T:%u", pers->plen, pers->tc);
        }
    else if (icmp6->icmp6_code == 0 && icmp6->icmp6_seq == htons(1)) {
            pers = &tmpl->person6->ie[1];
            tc = pers->tc;
            syslog(LOG_DEBUG, "found IE2 nmap fingerprinting probe P:%u T:%u", pers->plen, pers->tc);
     }
    }

    /* drop package if personality is set and plen and tc are -1 */
    if (!personality6_ip6_response(pers))
        return;

    /* use nmap fingerprint for packet length */
    original_payload_len = request_pkt_len - ETH_HDR_LEN - IP6_HDR_LEN - sizeof(struct icmp6_hdr);
    if (pers != NULL)
        data_field_len = pers->plen - sizeof(struct icmp6_hdr);
    else {
    ...
    if (original_payload_len >= data_field_len) {
        memcpy(icmp_response_pkt + sizeof(struct icmp6_hdr), request_data_field, data_field_len);
    }
    else {
        memcpy(icmp_response_pkt + sizeof(struct icmp6_hdr), request_data_field, original_payload_len);
        memset(icmp_response_pkt + sizeof(struct icmp6_hdr) + original_payload_len, 0, data_field_len - original_payload_len);
    }
    ...
    icmp6_send_pkt(inter, tmpl->ethernet_addr, NULL,
                   ...
                   icmp_response_pkt, data_field_len + sizeof(struct icmp6_hdr), tc);

}

void handle_node_information_query(const struct interface *inter, struct ip6_hdr *ip6,
                                   struct icmp6_hdr *icmp6)
{
    struct addr src_ip_addr, dst_ip_addr;
    struct template *tmpl = NULL;
    struct ni_message *ni_query = (struct ni_message *) icmp6;
    uint8_t tc = 0;
    ...
    /* Test for nmap fingerprint NI */
    struct ip_personate6 *pers = NULL;
    if (tmpl != NULL && tmpl->person6 != NULL) {
        if (ni_query->ni_code == 0 && ntohs(ni_query->ni_qtype) == 4) {
            pers = &tmpl->person6->ni;
            tc = pers->tc;
            syslog(LOG_DEBUG, "found NI nmap fingerprinting probe PLEN: %u TC: %u", pers->plen, pers->tc);
            if (!personality6_ip6_response(pers))
                return;
        }
    }

    if (pers == NULL || pers->plen == (uint16_t) -1) {
        /* if no personality6 is found or plen is -1 we only send the ni header */
        response_pkt_len = sizeof(struct ni_message);
    }
    else {
        if (pers->plen < sizeof(struct ni_message))
            response_pkt_len = sizeof(struct ni_message);
        else
            response_pkt_len = pers->plen;
    }

    u_char icmp_response_pkt[response_pkt_len];
    struct ni_message *ni_reply = (struct ni_message *) &icmp_response_pkt[0];
    ni_reply->ni_type = 140;
    ni_reply->ni_qtype = ni_query->ni_qtype;
    ni_reply->ni_nonce[0] = ni_query->ni_nonce[0];
    ni_reply->ni_nonce[1] = ni_query->ni_nonce[1];

    if (pers == NULL) {
        /* if no personality6 is found we reject the query */
        ni_reply->ni_code = 1;
    }
    else {
        /* TODO: implement real data response */
        ni_reply->ni_code = 0;
        memset(&icmp_response_pkt[0] + sizeof(struct ni_message), 0, response_pkt_len - sizeof(struct ni_message));
    }
   icmp6_send_pkt(inter, tmpl->ethernet_addr, NULL,
            (struct in6_addr *) &ip6->ip6_dst, (struct in6_addr *) &ip6->ip6_src,
            icmp_response_pkt, response_pkt_len, tc);
}
                      ...
                      int code)
{
    ...
    /* use U1 nmap test if it exists  */
    struct ip_personate6 *pers = NULL;
    if (tmpl != NULL && tmpl->person6 != NULL) {
        pers = &tmpl->person6->u1;
        tc = pers->tc;
        syslog(LOG_DEBUG, "found U1 nmap fingerprinting probe PLEN: %u TC: %u", pers->plen, pers->tc);
        if (!personality6_ip6_response(pers))
            return;
    }
    ...
    if (pers != NULL && pers->plen != (uint16_t) -1 && is_invoking_icmp_fitting_in_mtu(pers->plen)){
        response_pkt_len = pers->plen;
    }
    else if (is_invoking_icmp_fitting_in_mtu(invoking_pkt_len))
    {
...
int add_host_to_multicast_group(struct addr *host, struct addr *multicast_addr)
{
    ...
    syslog(LOG_DEBUG, "INFO: added %s to multicast group %s", addr_ntoa(host),
