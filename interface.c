...
interface_new(char *dev)
{
	...
	/* we need to increase (multiply) this value in order to be able to find the ipv6 adresses to */
	inter->if_ent.intf_len = sizeof(struct intf_entry) + sizeof(struct addr)*10;/* 10 is the number of aliases */
    ...
    int naddresses, char **addresses)
{
	...
	/* I added ip6 to the filter in order to receive ipv6 messages */
...
interface_init(char *dev, int naddresses, char **addresses)
{
	...
	/* TODO: this filter should not filter ipv6 requests */
